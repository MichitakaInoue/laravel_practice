                  ...
                  <div class="card-body text-center drill-body">
                    ...
                    <template v-if="isStarted && !isCountDown && !isEnd">
                      ...
                      <p class="problem-text">
                        <span v-for="(word, index)in problemWords" :class="{"text-promary": index < currentWordNum}">{{word}}</span>
                      </p>
                    </template>
                    <template>
                        <p>あなたのスコア</p>
      ...
      computed: { //リアルタイムで変更していくので監視するデータ　自動で監視しているので次の問題へ行く
        　problemWords: function(){//問題文自体
          ...
          totalWordNum: function(){ //実際の問題の文字数 　　computedで自動監視しているため、この時点でproblemKeyCodesは可変の値
                returnn this.problemKeyCodes.length　　
          },
          typingScore: function(){  //あなたのスコアで使われる　　
              return (this.wpm  * 2) * (1- this.miss.Num / (this.wpm * 2)) //正答率を考える
          }
      },
      methods: { //vue.jsで使える関数　監視されない
        ...
        countDown: function(){
            ...
            let timer = window.setInterval(() => {
                ...
                if(this.ountDownNum <=0){　//カウントダウンが0になったら次の処理に移っていく    
        ...
        showFirstProblem: function(){ //問題を提示
            ...
            $(window).on('keypress', e => {  
                console.log(e.which) //whichの中にキー入力された番号が出てくる
                ...
                console.log('現在回答しているのは何文字目:' + this.currentWordNum)
              }
            })
        },
